...
void scheduler(void)
{
  ...
  for (;;)
  {
    ...
    for (p2 = proc; p2 < &proc[NPROC]; p2++)
    {
      ...
      {
        acquire(&p2->lock);
        if (p2->state == RUNNABLE && p2->present_q != -1)
        {
          if (p2->wtime >= 30)
          {
            if (p2->q_no != 0)
            {
              pop_mid(&mlfq[p2->q_no], p2, p2->q_no);
              p2->q_no--;
              p2->present_q = -1;
              // p2->wtime = 0;
              push(&mlfq[p2->q_no], p2, p2->q_no);
            }
            // p2->present_q = -1;
            // push(&mlfq[p2->q_no], p2, p2->q_no);
          }
...
void push(struct QUEUE *q, struct proc *p, int qn)
{
  // printf("push\n");
  p->q_no = qn;
  p->wtime = 0;
...
void pop_mid(struct QUEUE *q, struct proc *p, int qn)
{
  if (q->size == 0)
  {
    return;
  ...
  else
  {
    ...
    //   {
    //     q->process[i] = q->process[(i + 1) % NPROC];
    //   }
    //   i = i % NPROC;
    // }
